            ...
            if isinstance(prev_tool_msg, ToolMessage) and isinstance(prev_ai_msg, AIMessage):
                ...
                for tool in current_tools:
                    if tool in target_tools and tool in prev_tools:
                        print(f"재귀 루프 감지! ({tool} 연속 호출) -> 강제 종료")
                        return "EditorAgent"
...
async def execute_tools(state: AgentState, current_itinerary: List[Dict]):
    ...
    if current_stage == "planning":
        ...
        if places_only:
            ...
            if (last_day >= total_days) and (count >= max_places):
                ...
                print(f"일정 가득 참 감지 (Day {last_day}, {count}/{max_places}). 검색 차단 예정.")
            ...
            if tgt:
                ...
                if best_match_idx != -1 and highest_score > 0.5:
                    if best_match_idx > 0:
                        ...
                        print(f"앵커 변경: '{tgt}' 삭제 -> 직전 장소 '{dynamic_anchor}' 기준 검색")
                    else:
                        dynamic_anchor = state.get('current_anchor') or state.get('destination')
                        print(f"앵커 변경: 첫 장소 삭제 -> 출발지 '{dynamic_anchor}' 기준 검색")
                    break
    ...
    for tool_call in tool_calls:
        ...
        if tool_name == "find_and_select_best_place":
            if is_schedule_full:
                print("일정 초과로 인한 검색 도구 실행 차단 (Block)")
                stop_msg = (
...
def process_deletions(tool_results, itinerary):
    ...
    for tool_message, tool_name, raw_json_output in tool_results:
        ...
        if tool_name in ["delete_place", "replace_place"]:
            try:
                print(f"도구 호출 확인: {tool_name}")
                data = json.loads(raw_json_output)
                tgt = data.get('place_name') or data.get('old')
                
                if tgt:
                    tgt_norm = normalize_name(tgt)
                    print(f"삭제 대상(원본): '{tgt}'")
                    print(f"삭제 대상(정규화): '{tgt_norm}'")
                    
                    best_match_idx = -1
                    highest_ratio = 0.0
                    
                    print(f">> 매칭 탐색 시작 (총 {len(places_only)}개 장소)")
                    ...
                    print(f">> 탐색 종료. 최고 점수: {highest_ratio:.4f}, 인덱스: {best_match_idx}")
                    ...
                    if best_match_idx != -1 and highest_ratio > 0.5:
                        ...
                        print(f"삭제 확정! Index {best_match_idx}: '{target_place['name']}'")
                        places_only.pop(best_match_idx)
                        is_still_there = any(p.get('name') == deleted_name for p in places_only)
                        if not is_still_there:
                            print(f" 확인 사살 완료. '{deleted_name}'은(는) 리스트에서 완전히 사라졌습니다.")
                        else:
                            print(f" 경고! '{deleted_name}'이(가) 아직 리스트에 남아있습니다. (동명이인 주의)")
                        modification_happened = True
                    else:
                        print(f"삭제 실패: 매칭되는 장소가 없거나 점수가 너무 낮음.")
...
def process_additions(tool_results, itinerary: List[Dict], empty_slot_info, state: AgentState):
    print(f" 받은 empty_slot_info: {empty_slot_info}")
    updated_itinerary = [x for x in itinerary if x.get('type') != 'move']
    print(f"장소 리스트(Before): {[x.get('name') for x in updated_itinerary]}")
    current_stage = state.get("dialog_stage", "planning")
        ...
        if tool_name == "find_and_select_best_place":
            try:
                ...
                if not place_name or place_name == "추천 장소 없음": 
                    ...
                    print("유효하지 않은 장소 데이터 -> Skip")
                    continue
                
                modification_happened = True

                if empty_slot_info:
                    print(f"빈자리 정보 감지됨! 로직 진입")
                    ...
                    if target_idx > len(updated_itinerary):
                        print(f" 인덱스 초과 ({target_idx} > {len(updated_itinerary)}) -> 맨 뒤로 보정")
                        target_idx = len(updated_itinerary)
                    
                    item_json['day'] = target_day
                    
                    updated_itinerary.insert(target_idx, item_json)
                    print(f" Insert 수행 완료! 이름: {place_name} -> Index: {target_idx}")
                    print(f" 삽입 직후 리스트: {[x.get('name') for x in updated_itinerary]}")
                    print(f" 빈자리(Place Index {target_idx})에 '{place_name}' 삽입 성공!")
                    ...
                    if not replaced:
                        ...
                        if count >= max_places:
                            if last_day >= total_days:
                                print(f"  일정 가득 참 (Day {last_day}, {count}곳). '{place_name}' 추가 거부.")
                                is_full_stop = True
                                continue 
                            else:
                        ...
                        updated_itinerary.append(item_json)
                        print(f"  Planning Append: {place_name} (Day {day_to_add})")
                else:
                    if empty_slot_info:
                        ...
                        print(f":  빈자리(Index {insert_idx})에 '{place_name}' 삽입")
                        empty_slot_info = None 
                    else:
                        target_day = 1
    ...
    print(f" 종료. 최종 리스트: {[x.get('name') for x in updated_itinerary]}\n")
...
def reorganize_itinerary_planning(items):
    ...
    for day in days:
        ...
        if day == 1:
            if rests: sorted_day.append(rests.pop(0)) 
            sorted_day.extend(cafes)                 
            sorted_day.extend(tours)                 
            sorted_day.extend(rests)                  
        else:
            sorted_day.extend(tours)
            if rests: sorted_day.append(rests.pop(0)) 
            sorted_day.extend(cafes)
...
async def call_tools_node(state: AgentState):
    ...
    if is_full_stop and tool_outputs:
        print(" Full Stop 신호 감지 -> LLM 메시지 강제 변경")
